...
5. When performing real-time imaging with ADMIRE on a Verasonics Vantage system, the Verasonics GUI provides a control that allows the transmit focus to be updated in real-time. It is important to note that the t0 calculation depends upon the transmit focus, so it is recommended to not update the transmit focus in real-time. This is because the t0 is calculated and used to generate the ADMIRE models that are used during real-time imaging as previously described. If a different transmit focus is desired, then it is recommended to update the transmit focus, calculate the new t0, and generate the ADMIRE models again. If the transmit focus is updated in real-time, then the t0 used to generate the ADMIRE models will most likely not be valid anymore. Therefore, this may cause degradation in image quality.
