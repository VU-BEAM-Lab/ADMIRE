...
// Description of ADMIRE_GPU_linear_probe_Verasonics_RF_buffer_data_type.cu: 
// This file contains the MEX-interface that calls the C/CUDA code for 
// performing ADMIRE on a GPU. It is used when params.data_type = 'Verasonics RF Buffer'
// and params.probe_type = 'Linear'.
...
static int num_buffer_rows;                      // Number of rows in the Verasonics RF buffer
static int num_depths;                           // Number of depth sample samples
static int total_elements_on_probe;              // Total number of elements on the transducer array
...
// GPU device arrays
static int16 * Verasonics_RF_buffer_data_d;    // Stores the Verasonics RF buffer data
...
cudaFree(Verasonics_RF_buffer_data_d);
cudaFree(reshaped_d);
cudaFree(delays_d);
...
if (!initialized) {
   ...
   num_buffer_rows = (int)GPU_fixed_params_h[1];
   num_depths = (int)GPU_fixed_params_h[2];
   total_elements_on_probe = (int)GPU_fixed_params_h[3];
   num_elements = (int)GPU_fixed_params_h[4];
   num_beams = (int)GPU_fixed_params_h[5];
   start_depth_offset = (int)GPU_fixed_params_h[6];
   stft_num_zeros = (int)GPU_fixed_params_h[7];
   stft_num_windows = (int)GPU_fixed_params_h[8];
   stft_window_shift = (int)GPU_fixed_params_h[9];
   stft_length = (int)GPU_fixed_params_h[10];
   max_windows_per_set = (int)GPU_fixed_params_h[11];
   num_selected_freqs = (int)GPU_fixed_params_h[12];
   num_corresponding_negative_freqs = (int)GPU_fixed_params_h[13];
   num_fits = (int)GPU_fixed_params_h[14];
   total_num_cropped_y_observations = (int)GPU_fixed_params_h[15];
   total_num_X_matrix_values = (int)GPU_fixed_params_h[16];
   total_num_B_values = (int)GPU_fixed_params_h[17];
   channel_data_output_flag = (int)GPU_fixed_params_h[18];
   ...
   cudaMalloc(&Verasonics_RF_buffer_data_d, num_buffer_rows * total_elements_on_probe * sizeof(int16));
...
GPU_adjustable_params_h = (float*)mxGetData(prhs[13]);
...
int16 * data_h;
data_h = (int16*)mxGetData(prhs[14]);
...
cudaMemcpy(Verasonics_RF_buffer_data_d, data_h, num_buffer_rows * total_elements_on_probe * sizeof(int16), cudaMemcpyHostToDevice);

// Define the grid and block dimensions for the reshaped_data GPU kernel
dim3 RESHAPE_GRID_SIZE;
RESHAPE_GRID_SIZE = dim3(num_depths + t0 - 1, num_beams, 1); 
dim3 RESHAPE_BLOCK_SIZE;
RESHAPE_BLOCK_SIZE = dim3(num_elements, 1, 1);

// Call the reshape_data GPU kernel in order to reshape the Verasonics RF buffer data and convert its values from the int16 data type to the float data type
reshape_data<<<RESHAPE_GRID_SIZE, RESHAPE_BLOCK_SIZE>>>(reshaped_d, Verasonics_RF_buffer_data_d, t0, num_buffer_rows, num_depths, num_elements); 
...
// Calculate the inverse short-time Fourier transform (the window overlap is assumed to be 0 for GPU execution, so the inverse fast fourier transform along each column of each STFT window just needs to be calculated in order to obtain the inverse short-time Fourier transform)
...
float * normalized_log_compressed_envelope_h;
float * delayed_data_h;

// Allocate the normalized_log_compressed_envelope_h array (this is the output array for the normalized and log-compressed envelope data)
plhs[0] = mxCreateNumericMatrix(num_depths, num_beams, mxSINGLE_CLASS, mxREAL);
normalized_log_compressed_envelope_h = (float*)mxGetData(plhs[0]);
...
// Transfer the normalized and log-compressed envelope data
cudaMemcpy(normalized_log_compressed_envelope_h, normalized_log_compressed_envelope_d, num_depths * num_beams * sizeof(float), cudaMemcpyDeviceToHost);
