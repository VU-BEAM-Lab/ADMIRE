...
// Description of ADMIRE_GPU_linear_probe_reshaped_data_type.cu: 
...
if (!initialized) {
   ...
   channel_data_output_flag = (int)GPU_fixed_params_h[16];
...
float * data_h;
data_h = (float*)mxGetData(prhs[14]);
...
// Perform the inverse short-time Fourier transform (the window overlap is assumed to be 0 for GPU execution, so the inverse fast fourier transform along each column of each STFT window just needs to be calculated in order to obtain the inverse short-time Fourier transform)
