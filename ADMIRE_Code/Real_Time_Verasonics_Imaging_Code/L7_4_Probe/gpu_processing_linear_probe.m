...
% Description of gpu_processing_linear_probe.m:
...
function gpu_processing_linear_probe(RcvData)
    ...
    save('Dataset_Linear_Probe.mat', 'RcvData', 'Trans', 'Receive', 'TW', ...
        'TX', 'P', 'Resource');
    %}
    ...
    persistent flag_1 GPU_fixed_params_h delays_h stft_window_h selected_freq_inds_h ...
        ...
        X_matrix_h B_thread_stride_h GPU_adjustable_params_h axial_positions ...
        lateral_positions ADMIRE_handle ax1 flag_2 frameCount startTime;
    
    % Obtain persistent variables (only need to do this once because most of
    % variables are fixed during scanning)
    if isempty(flag_1)
        flag_1 = 1;
        load ADMIRE_Variables_Linear_Probe_Verasonics_RF_Buffer_Data_Type.mat;
        ...
        axial_positions = depths .* 100;
        lateral_positions = beam_pos_x .* 100;
    end
    ...
    % Obtain the normalized and log-compressed envelope of the data that is 
    % processed with ADMIRE
    [envelope] = ADMIRE_GPU_linear_probe_Verasonics_RF_buffer_data_type(GPU_fixed_params_h, ...
        ...
        observation_thread_stride_h, num_predictors_h, X_matrix_thread_stride_h, ...
        X_matrix_h, B_thread_stride_h, GPU_adjustable_params_h, RcvData);
